...
Ray Camera::to_ray(float x, float y) {
    glm::vec3 offset = (m_right * random_float(-1.0f, 1.0f) + m_up * random_float(-1.0f, 1.0f)) * m_lens_radius;
    glm::vec3 origin = m_position + offset;
    glm::vec3 prefect_direction = normalize(m_top_left + x * m_horizontal - y * m_vertical);
