...
constexpr AXIS toAxis(int i) { return static_cast<AXIS>(i % 2); }

struct SphereLess {
  AXIS axis;

  bool operator()(const std::shared_ptr<Sphere> &s1,
                  const std::shared_ptr<Sphere> &s2) const {
    switch (axis) {
    case X:
      return s1->position().x < s2->position().x;
    // case Y: return s1->position().y < s2->position().y;
    case Z:
      return s1->position().z < s2->position().z;
    }
...
size_t
BVH::coarseIntersect(std::vector<std::shared_ptr<Sphere>> &potiential_spheres,
                     const Ray &ray) const {
