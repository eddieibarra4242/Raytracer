...
struct Color {
  ...
  static inline Color to_color(const glm::vec3 &color) {
    glm::vec3 ranged_color =
      glm::clamp(glm::pow(color, glm::vec3(1.0f / GAMMA)), 0.0f, 1.0f) * 255.0f;
    return Color{static_cast<uint8_t>(ranged_color.r),
                 static_cast<uint8_t>(ranged_color.g),
                 static_cast<uint8_t>(ranged_color.b), 255};
  }
};
...
public:
  ...
  [[nodiscard]] inline Color read(uint32_t x, uint32_t y) const {
    return get_wrapped(x, y);
  }
